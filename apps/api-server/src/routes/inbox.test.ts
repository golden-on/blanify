...
describe.skipIf(!reachable)("Inbox REST routes", () => {
  ...
  it("lists threads with correct unread counts for the owning tenant", async () => {
    const app = buildApp();
    const response = await app.inject({ method: "GET", url: `/api/v1/inbox/threads?accountId=${account.id}` });
  ...
  it("returns no threads when queried under a different tenant's accountId", async () => {
    const app = buildApp();
    const response = await app.inject({ method: "GET", url: `/api/v1/inbox/threads?accountId=${otherAccount.id}` });
  ...
  it("posts a host message, bumps lastMessageAt, and it appears in message history", async () => {
    ...
    const postResponse = await app.inject({
      ...
      payload: { accountId: account.id, content: "Thanks for reaching out!" },
    });
    ...
    const historyResponse = await app.inject({
      method: "GET",
      url: `/api/v1/inbox/threads/${thread.id}/messages?accountId=${account.id}`,
    });
